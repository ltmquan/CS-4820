...
(solver-push)
(z3-assert (alice :bool bob :bool clara :bool)
           (and
            (= bob (= alice (= (+ (ite alice 1 0)
                                  (ite bob 1 0)
                                  (ite clara 1 0)) 
			       1)))
            (= clara (not bob))))
...
(solver-push)
(z3-assert (mr-fireman :int mr-guard :int mr-driver :int)
           (and
            (>= mr-fireman 0) (<= mr-fireman 2)
            (>= mr-guard 0) (<= mr-guard 2)
            (>= mr-driver 0) (<= mr-driver 2)
            (not (= mr-fireman mr-guard))
            (not (= mr-fireman mr-driver))
            (not (= mr-guard mr-driver))
            (= 1 (+ (ite (not (= mr-driver 1)) 1 0)
                    (ite (not (= mr-fireman 2)) 1 0)
                    (ite (= mr-driver 2) 1 0)
                    (ite (not (= mr-fireman 1)) 1 0)))))
(check-sat)
(solver-pop)
...
(defun magic-square-diag-constraints (sum-var)
  (let ((main-diag (loop for i below 3 collect (get-3x3-magic-square-var i i)))
        (anti-diag (loop for i below 3 collect (get-3x3-magic-square-var i (- 2 i)))))
    `(and (= ,sum-var (+ . ,main-diag))
          (= ,sum-var (+ . ,anti-diag)))))
 
(defun 3x3-magic-square-all-distinct ()
  (let ((all-vars (loop for row below 3 append
                        (loop for col below 3
                              collect (get-3x3-magic-square-var row col)))))
    `(distinct . ,all-vars)))
...
(z3-assert-fn (3x3-magic-square-var-specs 'S)
              (3x3-magic-square-vars-between-1-and-9))
(z3-assert-fn (3x3-magic-square-var-specs 'S)
              (magic-square-diag-constraints 'S))
(z3-assert-fn (3x3-magic-square-var-specs 'S)
              (3x3-magic-square-all-distinct))
(check-sat)
(get-model-as-assignment)
(solver-pop)
...
(defun sudoku-at-least-one-constraints ()
  (cons 'and
        (loop for row below 9 append
              (loop for col below 9 collect
                    `(or ,@(loop for val from 1 to 9
                                 collect (sudoku-cell-var row col val)))))))
 
(defun sudoku-at-most-one-constraints ()
  (cons 'and
        (loop for row below 9 append
              (loop for col below 9 append
                    (loop for v1 from 1 to 9 append
                          (loop for v2 from (1+ v1) to 9
                                collect `(not (and ,(sudoku-cell-var row col v1)
                                                   ,(sudoku-cell-var row col v2)))))))))
 
(defun sudoku-row-constraints ()
  (cons 'and
        (loop for row below 9 append
              (loop for val from 1 to 9 collect
                    `(or ,@(loop for col below 9
                                 collect (sudoku-cell-var row col val)))))))
  ...
  (cons 'and
        (loop for box-row below 3 append
              (loop for box-col below 3 append
                    (loop for val from 1 to 9 collect
                          `(or ,@(loop for r below 3 append
                                       (loop for c below 3
                                             collect (sudoku-cell-var
                                                      (+ (* box-row 3) r)
                                                      (+ (* box-col 3) c)
                                                      val)))))))))
 
(defun sudoku-starting-board-constraints (board)
  (cons 'and
        (loop for i below 81
              for cell = (nth i board)
              for row = (floor i 9)
              for col = (mod i 9)
              when (not (equal cell '_))
              collect (sudoku-cell-var row col cell))))
 
(defun solve-sudoku (input-grid)
  (solver-init)
  (solver-push)
  (z3-assert-fn (sudoku-var-specs) (sudoku-at-least-one-constraints))
  (z3-assert-fn (sudoku-var-specs) (sudoku-at-most-one-constraints))
  (z3-assert-fn (sudoku-var-specs) (sudoku-row-constraints))
  (z3-assert-fn (sudoku-var-specs) (sudoku-col-constraints))
  (z3-assert-fn (sudoku-var-specs) (sudoku-box-constraints))
  (z3-assert-fn (sudoku-var-specs) (sudoku-starting-board-constraints input-grid))
...
(defun sudoku-int-cell-var (row col)
  (intern (concatenate 'string "C" (write-to-string row) "_" (write-to-string col))))
 
(defun sudoku-int-var-specs ()
  (loop for row below 9 append
        (loop for col below 9 append
              `(,(sudoku-int-cell-var row col) :int))))
 
(defun sudoku-int-range-constraints ()
  (cons 'and
        (loop for row below 9 append
              (loop for col below 9 append
                    `((>= ,(sudoku-int-cell-var row col) 1)
                      (<= ,(sudoku-int-cell-var row col) 9))))))
 
(defun sudoku-int-row-constraints ()
  (cons 'and
        (loop for row below 9 collect
              `(distinct ,@(loop for col below 9
                                 collect (sudoku-int-cell-var row col))))))
  ...
  (cons 'and
        (loop for box-row below 3 append
              (loop for box-col below 3 collect
                    `(distinct ,@(loop for r below 3 append
                                       (loop for c below 3
                                             collect (sudoku-int-cell-var
                                                      (+ (* box-row 3) r)
                                                      (+ (* box-col 3) c)))))))))
 
(defun sudoku-int-starting-constraints (board)
  (cons 'and
        (loop for i below 81
              for cell = (nth i board)
              for row = (floor i 9)
              for col = (mod i 9)
              when (not (equal cell '_))
              collect `(= ,(sudoku-int-cell-var row col) ,cell))))
 
(defun solve-sudoku-alternate (input-grid)
  (solver-init)
  (solver-push)
  (z3-assert-fn (sudoku-int-var-specs) (sudoku-int-range-constraints))
  (z3-assert-fn (sudoku-int-var-specs) (sudoku-int-row-constraints))
  (z3-assert-fn (sudoku-int-var-specs) (sudoku-int-col-constraints))
  (z3-assert-fn (sudoku-int-var-specs) (sudoku-int-box-constraints))
  (z3-assert-fn (sudoku-int-var-specs) (sudoku-int-starting-constraints input-grid))
  (let ((result (check-sat)))
    (if (equal result :unsat)
        (progn (solver-pop) 'unsat)
        (let ((model (get-model-as-assignment)))
...
(defun get-square-value (soln row col)
  (cadr (assoc-equal (sudoku-int-cell-var row col) soln)))
...
;; this is an arbitrarily hard sudoku taken from the internet
...
(defun sudoku-cell-var-n (row col val)
  (intern (concatenate 'string "X"
                       (write-to-string row) "_"
                       (write-to-string col) "_"
                       (write-to-string val))))

(defun sudoku-var-specs-n (n)
  (loop for row below n append
        (loop for col below n append
              (loop for val from 1 to n append
                    `(,(sudoku-cell-var-n row col val) :bool)))))

(defun sudoku-at-least-one-constraints-n (n)
  (cons 'and
        (loop for row below n append
              (loop for col below n collect
                    `(or ,@(loop for val from 1 to n
                                 collect (sudoku-cell-var-n row col val)))))))

(defun sudoku-at-most-one-constraints-n (n)
  (cons 'and
        (loop for row below n append
              (loop for col below n append
                    (loop for v1 from 1 to n append
                          (loop for v2 from (1+ v1) to n
                                collect `(not (and ,(sudoku-cell-var-n row col v1)
                                                   ,(sudoku-cell-var-n row col v2)))))))))

(defun sudoku-row-constraints-n (n)
  (cons 'and
        (loop for row below n append
              (loop for val from 1 to n collect
                    `(or ,@(loop for col below n
                                 collect (sudoku-cell-var-n row col val)))))))

(defun sudoku-col-constraints-n (n)
  (cons 'and
        (loop for col below n append
              (loop for val from 1 to n collect
                    `(or ,@(loop for row below n
                                 collect (sudoku-cell-var-n row col val)))))))

(defun sudoku-box-constraints-n (n)
  (let ((box-size (isqrt n)))
    (cons 'and
          (loop for box-row below box-size append
                (loop for box-col below box-size append
                      (loop for val from 1 to n collect
                            `(or ,@(loop for r below box-size append
                                         (loop for c below box-size
                                               collect (sudoku-cell-var-n
                                                        (+ (* box-row box-size) r)
                                                        (+ (* box-col box-size) c)
                                                        val))))))))))

(defun sudoku-starting-board-constraints-n (board n)
  (cons 'and
        (loop for i below (* n n)
              ...
              for col = (mod i n)
              when (not (equal cell '_))
              collect (sudoku-cell-var-n row col cell))))

(defun get-square-value-n (soln row col n)
  (block outer
    (loop for i from 1 to n
          do (when (and (cdr (assoc-equal (sudoku-cell-var-n row col i) soln))
                        (cadr (assoc-equal (sudoku-cell-var-n row col i) soln)))
               (return-from outer i)))
    nil))

(defun pretty-print-sudoku-solution-n (soln n)
  (let ((box-size (isqrt n)))
    (loop for row below n
          do (progn (terpri)
                    (loop for col below n
                          do (progn (format t "~2A " (get-square-value-n soln row col n))
                                    (when (equal (mod col box-size) (1- box-size))
                                      (format t "  "))))
                    (when (equal (mod row box-size) (1- box-size))
                      (terpri))))))

(defun solve-sudoku-n (input-grid n)
  (solver-init)
  (solver-push)
  (z3-assert-fn (sudoku-var-specs-n n) (sudoku-at-least-one-constraints-n n))
  (z3-assert-fn (sudoku-var-specs-n n) (sudoku-at-most-one-constraints-n n))
  (z3-assert-fn (sudoku-var-specs-n n) (sudoku-row-constraints-n n))
  (z3-assert-fn (sudoku-var-specs-n n) (sudoku-col-constraints-n n))
  (z3-assert-fn (sudoku-var-specs-n n) (sudoku-box-constraints-n n))
  (z3-assert-fn (sudoku-var-specs-n n) (sudoku-starting-board-constraints-n input-grid n))
  (let ((result (check-sat)))
    (if (equal result :unsat)
        (progn (solver-pop) 'unsat)
        (let ((model (get-model-as-assignment)))
...
(defparameter *sudoku-16x16-example*
  '( _  _  _  13    _  _  _  _    _  _  _  _   14  _  _  _
     _  _  _  _     _  _  2  _    _  8  _  _    _  _  _  _
     _  _  _  _     _  4  _  _    _  _  13 _    _  _  _  _
     _  3  _  _     _  _  _  15   6  _  _  _    _  _  11 _

     _  _  6  _     _  _  _  _    _  _  _  _    _  _  _  4
     _  _  _  _     _  _  7  _    _  3  _  _    _  10 _  _
     _  _  _  _     _  _  _  _    _  _  _  _    _  _  _  _
     8  _  _  _     9  _  _  _    _  _  7  _    _  _  _  3

     5  _  _  _     _  _  8  _    _  _  12 _    _  _  _  9
     _  _  _  _     _  _  _  _    _  _  _  _    _  _  _  _
     _  _  10 _     _  6  _  _    _  _  _  _    _  _  _  _
     16 _  _  _     _  _  _  12   11 _  _  _    _  _  6  _

     _  14 _  _     _  _  _  6    15 _  _  _    _  _  3  _
     _  _  _  _     _  _  13 _    _  5  _  _    _  _  _  _
     _  _  _  _     _  7  _  _    _  _  4  _    _  _  _  _
     _  _  _  4     _  _  _  _    _  _  _  _    9  _  _  _ ))

(pretty-print-sudoku-solution-n
  (time (solve-sudoku-n *sudoku-16x16-example* 16))
  16)
